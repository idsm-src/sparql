...
public final class RyuFloat
{
    ...
    public static String floatToString(float value)
    {
        ...
        if(dpIsTrailingZeros && !acceptBounds)
            dp--;

        while(dp / 10 > dm / 10)
        {
            if(dp < 100)
                break; // We print at least two digits, so we might as well stop now.
        ...
        if(dvIsTrailingZeros && (lastRemovedDigit == 5) && (dv % 2 == 0))
            lastRemovedDigit = 4; // Round down not up if the number ends in X50000 and the number is even.

        int output = dv + ((dv == dm && !(dmIsTrailingZeros && acceptBounds)) || (lastRemovedDigit >= 5) ? 1 : 0);
    ...
    /*
     * Returns the exponent of the largest power of 5 that divides the given value, i.e., returns i such that
     * value = 5^i * x, where x is an integer.
     */
     ...
     * [m * 5^(-e_2) / 10^q] = [m * 5^(-e_2 - q) / 2^q] = [m * [5^(p - q)/2^k] / 2^(q - k)] = [m * POW5[i] / 2^j].
     */
    ...
    /*
     * Compute the exact result of
     * [m * 2^p / 10^q] = [m * 2^(p - q) / 5 ^ q] = [m * [2^k / 5^q] / 2^-(p - q - k)] = [m * POW5_INV[q] / 2^j].
     */
