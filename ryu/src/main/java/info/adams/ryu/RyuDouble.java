...
public final class RyuDouble
{
    ...
    public static String doubleToString(double value)
    {
        ...
        if(dmIsTrailingZeros || dvIsTrailingZeros)
        {
            ...
            if(dvIsTrailingZeros && (lastRemovedDigit == 5) && (dv % 2 == 0))
                lastRemovedDigit = 4; // Round even if the exact numbers is .....50..0.

            output = dv + ((dv == dm && !(dmIsTrailingZeros && acceptBounds)) || (lastRemovedDigit >= 5) ? 1 : 0);
        }
        else
        {
            while(dp / 10 > dm / 10)
     ...
     * m * 5^p / 10^q = m * 5^(p - q) / 2^q = m * 5^i / 2^j, with q chosen such that m * 5^i / 2^j has sufficiently many
    ...
    /*
     * Compute the high digits of m / 5^i / 2^j such that the result is accurate to at least 9 decimal digits. i and j
     * are already chosen appropriately.
     */
