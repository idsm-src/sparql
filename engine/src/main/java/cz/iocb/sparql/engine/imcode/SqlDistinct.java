...
public final class SqlDistinct extends SqlIntercode
{
    ...
    public String translate(Request request)
    {
        ...
        for(VariableBinding v : bindings.getValues())
            for(ResourceClass c : v.getClasses())
                if(isStringLiteral(c))
                    stringLiterals.add(v.getVariable());
        ...
        for(Variable var : stringLiterals)
            hashColumns.addAll(child.getVariableBindings().get(new Variable("#hash_" + var)).getNonConstantColumns());
            ...
            Set<Variable> distinctVariables)
    {
        ...
        {
            List<Set<ResourceClass>> newKey = new ArrayList<>();

            for(Variable var : distinctVariables)
            {
                Set<ResourceClass> classes = new HashSet<>();

                VariableBinding unionBinding = union.getVariable(var);
                VariableBinding binding = child.getVariable(var);
            ...
            List<SqlIntercode> newValue = new ArrayList<>();
            newValue.add(child);
            ...
            sorts.add(new Pair<>(newKey, newValue));
        }
            ...
            Set<Variable> distinctVariables)
    {
        ...
        for(Variable var : distinctVariables)
        {
            VariableBinding binding = union.getVariable(var);

            if(binding != null && !binding.canBeNull() && binding.getClasses().size() == 1)
            {
                ResourceClass rc = binding.getClasses().iterator().next();
                int[] counts = new int[rc.getColumnCount()];

                for(SqlIntercode child : union.getChilds())
                    if(child.getMapping(var, rc) != null)
                        for(int i = 0; i < rc.getColumnCount(); i++)
                            if(child.getMapping(var, rc).get(i) instanceof ConstantColumn)
                                counts[i]++;
    ...
    public void generateExplanation(StringBuilder builder, String indent)
    {
        ...
        builder.append(distinctVariables.stream().map(v -> v.toString()).collect(joining(" ", " ", "")));
