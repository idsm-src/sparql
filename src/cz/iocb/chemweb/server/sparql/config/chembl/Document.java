...
class Document
{
    ...
    {
        config.addIriClass(new UserIriClass(schema, "document", Arrays.asList("bigint"),
                "http://rdf\\.ebi\\.ac\\.uk/resource/chembl/document/CHEMBL[1-9][0-9]*", SqlCheck.IF_MATCH));

        config.addIriClass(new UserIriClass(schema, "pubmed", Arrays.asList("bigint"),
                "http://identifiers\\.org/pubmed/[1-9][0-9]*"));
    }
