...
public abstract class CustomButton extends ButtonBase {
    ...
    switch (type) {
      ...
      case Event.ONMOUSEUP:
        if (isCapturing) {
          ...
          int x = Event.getCurrentEvent().getClientX() + Window.getScrollLeft();
          int y = Event.getCurrentEvent().getClientY() + Window.getScrollTop();
          boolean in = (x >= this.getAbsoluteLeft() && x <= this.getAbsoluteLeft() + this.getOffsetWidth()) &&
              (y >= this.getAbsoluteTop() && y <= this.getAbsoluteTop() + this.getOffsetHeight());

          if (/*isHovering() &&*/ in && event.getButton() == Event.BUTTON_LEFT) {
            onClick();
          }
        }
        break;
   ...
   */
  final public void setHovering(boolean hovering) {
